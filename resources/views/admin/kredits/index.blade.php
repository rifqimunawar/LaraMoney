    ...
    <div class="col-12">
      <div class="card mb-4">
        <div class="card-header pb-0">
          ...
          <a href="{{ route('kredit.create') }}" class="btn btn-danger btn-xs">Add Cost  -</a>
          <p class="text-xs text-secondary font-weight opacity-7" >
            {{-- Total Uang Masuk Rp: {{ $totalRp }}</p > --}}
            Total Uang Keluar Rp: {{ number_format($totalRp, 0, ',', '.') }} </p >
        </div>
            ...
            <table class="table align-items-center mb-0">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-xs">{{ $kredit->created_at->format('Y-m-d') }}</td>
                  <td class="text-xs">{{ number_format($kredit->rp, 0, ',', '.') }}</td>
                  <td class="text-xs">{{ $kredit->category->name }}</td>
                  <td class="text-xs">{{ $kredit->description }}</td>
                  <td class="text-center text-xs">
                    <form action="/dahsboard/kredit/{{ $kredit->id }}"method="POST">
                      <a class="btn btn-link text-dark px-3 mb-0" href="/dahsboard/kredit/{{ $kredit->id }}/edit"><i class="fas fa-pencil-alt text-dark me-2" aria-hidden="true"></i>Edit</a>
                      @csrf
                      @method('DELETE')
